...
export default function GuestsPage() {
  ...
  const [categoryFilter, setCategoryFilter] = useState<'all' | 'family' | 'friends' | 'colleagues' | 'other'>('all');
  ...
  const filteredGuests = guests.filter(guest => {
    ...
    const categoryMatch = categoryFilter === 'all' || guest.category === categoryFilter;
  ...
  const stats = {
    ...
    totalAttendees: guests.filter(g => g.rsvp_status === 'confirmed').length + 
  ...
  const updateRSVP = (guestId: string, status: 'confirmed' | 'declined' | 'pending') => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Gestion des Invités</h1>
      ...
      <div className="flex flex-wrap gap-4 items-center">
        <div className="flex gap-2">
          <span className="text-sm font-medium text-gray-700">Statut RSVP:</span>
          {(['all', 'confirmed', 'pending', 'declined'] as const).map(status => (
            ...
            >
              {status === 'all' ? 'Tous' : 
               ...
               status === 'pending' ? 'En attente' : 'Déclinés'}
            </Button>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          ...
          {(['all', 'family', 'friends', 'colleagues', 'other'] as const).map(category => (
            <Button
              key={category}
            ...
            >
              {category === 'all' ? 'Toutes' : 
               ...
               category === 'friends' ? 'Amis' :
               category === 'colleagues' ? 'Collègues' : 'Autres'}
            </Button>
          ))}
        </div>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-center p-3">Table</th>
              ...
              <tbody>
                {filteredGuests.map((guest) => (
                  <tr key={guest.id} className="border-b hover:bg-gray-50">
                    <td className="p-3">
                      <div>
                        ...
                        {guest.plus_one && guest.plus_one_name && (
                          <p className="text-sm text-gray-500">+ {guest.plus_one_name}</p>
                        )}
                        {guest.notes && (
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-1 text-xs rounded-full ${getCategoryColor(guest.category)}`}>
                    ...
                    <td className="p-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className={`px-2 py-1 text-xs rounded-full ${getRSVPColor(guest.rsvp_status)}`}>
                    ...
                    <td className="p-3">
                      <div className="flex flex-wrap gap-1">
                        {guest.dietary_restrictions.map((restriction, index) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="hover:shadow-md transition-shadow cursor-pointer">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="font-semibold text-gray-900">Relances automatiques</h3>
            <p className="text-sm text-gray-600">Envoyer des rappels aux invités</p>
          </CardContent>
        </Card>
