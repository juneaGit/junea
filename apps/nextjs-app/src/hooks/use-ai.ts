...
export function useAI() {
  ...
  const generateBudgetInsights = useCallback(
    async (
      ...
      try {
        return await aiService.generateBudgetInsights(
          userProfile,
          weddingProfile,
          currentBudget,
        );
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erreur lors de l'analyse du budget";
        setError(errorMessage);
        return [];
      } finally {
        setLoading(false);
  ...
  const generateTimeline = useCallback(
    async (userProfile: any, weddingProfile: any): Promise<any[]> => {
      setLoading(true);
      setError(null);

      try {
        return await aiService.generateTimelineRecommendations(
          userProfile,
          weddingProfile,
        );
      } catch (err) {
